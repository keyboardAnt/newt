"""Progress/aggregation helpers for TD-MPC2 training runs.

This module intentionally does *not* define liveness semantics.
Use `discover.liveness` for centralized liveness detection and task aggregation.
"""
...
def attach_max_step(df: "pd.DataFrame") -> "pd.DataFrame":
    """Add a 'max_step' column from the best available progress fields.

    This is purely a progress helper. Liveness logic lives in `discover.liveness`.
    """
    pd = require_pandas()
    ...
    def _get_step(row):
        # Prefer whichever source reports the furthest progress:
        # - local checkpoints (ckpt_step)
        # - wandb summary (_step)
        ckpt_step = pd.to_numeric(row.get("ckpt_step"), errors="coerce")
        wandb_step = None
        summary = row.get("summary")
        if isinstance(summary, dict):
            wandb_step = pd.to_numeric(summary.get("_step"), errors="coerce")

        candidates = [v for v in (ckpt_step, wandb_step) if pd.notna(v)]

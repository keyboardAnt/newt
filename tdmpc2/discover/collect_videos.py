...
def find_best_runs(logs_dir: Path, min_step: int) -> List[Dict]:
    ...
    ckpt_patterns = (
        "*/checkpoints/*.pt",          # legacy run-first: logs/<run_id>/checkpoints/*.pt
        "*/*/checkpoints/*.pt",        # task-first:       logs/<task>/<run_id>/checkpoints/*.pt
        "*/*/*/checkpoints/*.pt",      # older nested:     logs/<task>/<seed>/<run_id>/checkpoints/*.pt
    )
    for pat in ckpt_patterns:
        for ckpt in logs_dir.glob(pat):
